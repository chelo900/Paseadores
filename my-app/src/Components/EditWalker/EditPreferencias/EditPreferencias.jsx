...
function EditPreferencias() {
    ...
    const handleSubmit=(e)=>{
        e.preventDefault()
        if(preferencias?.length !==0){dispatch(putPreferencias(userId, newPreferencias))}
        if(preferencias?.length === 0)dispatch(sendPreferencias(newPreferencias))
        swal({title:'Tus preferencias fueron actualizados',
      icon: 'info'})
      setTimeout(() => {
    ...
    return (
      ...
      <form className={style.total}  onSubmit={(e) => handleSubmit(e)}>
          ...
          <div className={style.izquierda}>
            ...
            <label className={style.label}>
                ...
                Duración de los Paseos
                     <select className={style.select} onChange={e=>handleHours(e)} >
            ...
            <label className={style.label}>
                    ...
                    <input name="perros_por_paseo" onChange={e=>setNewPreferencias({...newPreferencias, perros_por_paseo:e.target.value})} placeholder="7,8,9"/>
                </label>
            </div>
            <div className={style.container}>
            <label className={style.label}>
                    Elegí en que momento del día trabajas
                    <select className={style.select} onChange={e=>handleTurnos(e)} >
            ...
            <label className={style.label}>
                ...
                Hora de comienzo jornada laboral:
                    <input onChange={e=>setNewPreferencias({...newPreferencias, comienzo_jornada:e.target.value+':00:00'})} placeholder="06,07,08"/>
            </label>
            </div>
          </div>
          <div className={style.derecha}>
            ...
            <label className={style.label}>
                    Hora final de joranda laboral:                        
                        <input onChange={e=>setNewPreferencias({...newPreferencias, fin_jornada:e.target.value+':00:00'})} placeholder="21,22"/>
                </label>    
        ...
        <div className={style.containerBtn}>
          <button className={style.btn} type="submit"> Enviar Preferencias </button>
        </div>
      </form> 
    </div>
    );
